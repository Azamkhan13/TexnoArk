...
function Product() {
  ...
  async function handleRate(e: any) {
    ...
    const response = await axios.post(
      "https://ecomapi.ilyosbekdev.uz/products/rate",
      payload,
      {
        headers: {
                  ...
                  <div className="flex flex-col gap-[20px] mt-[40px] mb-[50px]">
                    {commets.slice(0, 5).map((e: any, i: number) => {
                      return (
                        <div
                          key={i}
                          className="w-[100%] bg-white py-[30px] px-[40px] rounded-xl flex gap-[50px] items-start"
                        >
                          <div className="w-[15%]">
                            <Tooltip
                              title={`${e?.user_id?.last_name} ${e?.user_id?.first_name}`}
                              placement="top"
                            >
                              <Avatar
                          ...
                          <div className="w-[70%}">
                            <h4 className="font-bold text-[24px] mb-[10px] max-sm:text-[18px]">
                              {e?.user_id?.last_name} {e?.user_id?.first_name}
                            </h4>
                            <p className="font-medium text-[#240E00CC] text-[16px] max-sm:text-[13px]">
                              {e?.comment}
                            </p>
                          </div>
                        </div>
