  ...
  return (
    <div className="product-card justify-between">
      <img
        ...
        src={datas?.product_id?.images[0]}
        alt={datas.product_id}
      />
      ...
      <div className="product-actions">
        <h4 className="product-price">
          {datas?.product_id?.price * count} so‘m
        </h4>
        <div className="action-icons">
          <span onClick={() => handleDelete(datas?.id)} className="action-icon">
            <DeleteOutlined className="icon" />
          </span>
          <span
            onClick={() => handleLike(datas?.product_id?.id)}
